...
------
#### 2026/4/21 refactor 更新

**本次更新:**

- 依赖切到面向 Python 3.13 的新版本;
- 移除原有滑块识别/OCR链路, 登录阶段改为 cookies + 手动验证兜底;
- 主流程改成在课程母页扫描蓝条未满节次, 逐节锁定、播放、取消标记、循环重扫;
- 修复任务监控误报异常的问题;
- 清理无效依赖与旧的运行时安装逻辑.
...
4.运行 **Autovisor.exe**，程序会自动打开浏览器，登录后在课程母页扫描蓝条未满节次并循环处理。
...
- 现在仍然使用异步协程持续检测弹题，但站点结构变化时仍可能需要调整选择器。
